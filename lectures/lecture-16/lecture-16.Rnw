...
\begin{document}
  ...
  \begin{darkframes}
    ...
    \begin{frame}[fragile]{Making predictions}
      ...
      @
      <<include=F>>=
      prediction <- predict.glm(model, data.frame(fare=50), type="response")
      @
      \[
    ...
    \begin{frame}{Interpreting the coefficients}
      ...
      \begin{itemize}[<+->]
        \item When $\text{fare}=0$, we predict that the log odds will be $\Sexpr{b0}$ \pause, so the probability or survival is predicted to be $0.29$.
        \item When fare increases by \pounds 1, we predict that the log odds will decrease by $\Sexpr{abs(b1)}$.
      \end{itemize}
