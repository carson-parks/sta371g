...
\begin{document}
  ...
  oil <- read.csv('../../data/oil_monthly.csv') 
  @ 
  <<include=F, cache=F>>=
  ...
  \begin{darkframes}
    ...
    \begin{frame}[fragile]{Simple Exponential Smoothing for Oil Prices}
      \fontsize{8}{8}\selectfont
      Let's try using SES to predict the oil pricess.
      <<fig.height=2.5>>=
      price <- ts(oil$price, start=1991, frequency=12)
      price_ses_05 <- ses(price, alpha = 0.5)
      plot(price)
      lines(price_ses_05$fitted, col='green')
      @  
    \end{frame}
    ...
    \begin{frame}[fragile]{Simple Exponential Smoothing for Oil Prices}
      ...
      price_ses <- ses(price)
      plot(price)
      lines(price_ses$fitted, col='green')
      @  
    \end{frame}
    ...
    \begin{frame}[fragile]{Simple Exponential Smoothing for Oil Prices}
      \fontsize{8}{8}\selectfont
      <<fig.height=2.5, message=FALSE, warning=FALSE>>= 
      price_ses_h8 <- ses(price, h=8)
      plot(price_ses_h8, col='green')
      @
    \end{frame}
